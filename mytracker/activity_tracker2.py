...
def tcp_connection():
	...
	global IS_LOGIN,restriction,tcp_connection_flag
	global total_offline_time
	global live_data
	while not IS_LOGIN:
		time.sleep(0.25)
	...
	while IS_LOGIN:
		...
		try:
			if network_connection == True and tcp_connection_flag == True :
				s.send(live_data.encode())
				data = s.recv(1024)
				print("received data:", data.decode())
			...
			tcp_connection_flag == True
			print("conection problem ,please wait!")
			tcp_connection()
...
activity_number = 0
def sqldb():
	global IS_LOGIN,network_connection,live_data,mydb,mycursor,activity_number,records,idle_second,idle_minute,restricted_windows_dict,restriction,tcp_connection_flag
	while not IS_LOGIN:
		time.sleep(0.25)
	while IS_LOGIN:
		time.sleep(50)
		...
		if network_connection == False or tcp_connection_flag == False:
